...
class Channel_model extends CI_Model {
    ...
    function add_category(){
        $rs = $this->db->insert('tbl_channel_cat', array('name'=>$this->input->post('category_name')));
        
        if(!empty($rs)){
            $this->common->setMsg('admin/channel_category',sprintf($this->lang->line('msg_success_add'),"Category"));
            return TRUE;
        }
    }
    ...
    function add_channel(){
        ...
        if(isset($_FILES['file']['name']) && $_FILES['file']['name']!=''){
            ...
            if ( $this->upload->do_upload('file')){
                ...
                $this->image->resizeImage('88','65',"crop");
        ...
        $rs = $this->db->insert('tbl_channel',$arr);
        
        if(!empty($rs)){
            $this->common->setMsg('admin/list_channel',sprintf($this->lang->line('msg_success_add'),"Channel"));
            return TRUE;
        }        
    }
    ...
    function get_channel($package_id=0){
        if($package_id<=0){
            $rs = $this->db->get('tbl_channel')->result_array();
            return $rs;
        }else{
            $rs = $this->db->query("SELECT * FROM tbl_package_channel AS a LEFT JOIN tbl_channel AS b ON a.channel_id=b.channel_id WHERE a.package_id='".$package_id."'")->result_array();
            //echo "<pre>";print_r($rs);die;
            return $rs;            
        }
    }
    
    function getAddedList($package_id){
        $rs = $this->db->query("SELECT * FROM tbl_package_channel AS a LEFT JOIN tbl_channel AS b ON a.channel_id=b.channel_id WHERE a.package_id='".$package_id."'")->result_array();
        //echo "<pre>";print_r($rs);die;
        return $rs;
    }
    
    function getRemainList($package_id){
        $rs = $this->db->query("SELECT * FROM tbl_channel WHERE channel_id NOT IN(SELECT channel_id FROM tbl_package_channel WHERE package_id='".$package_id."')")->result_array();
        //echo "<pre>";print_r($rs);die;
        return $rs;        
    }
}
